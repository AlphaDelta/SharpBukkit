...
namespace net.minecraft.src
{
	public class NetLoginHandler : net.minecraft.src.NetHandler
	{
		...
		{
			net.minecraft.src.EntityPlayerMP entityplayermp = mcServer.configManager.Login(this
				, packet1login.username);
			if (entityplayermp != null)
			{
				...
				logger.Info((new java.lang.StringBuilder()).Append(GetUserAndIPString()).Append(" logged in with entity id "
					).Append(entityplayermp.entityId).Append(" at (").Append(entityplayermp.posX).Append
					(", ").Append(entityplayermp.posY).Append(", ").Append(entityplayermp.posZ).Append
					(")").ToString());
				...
				net.minecraft.src.NetServerHandler netserverhandler = new net.minecraft.src.NetServerHandler
					(mcServer, netManager, entityplayermp);
				netserverhandler.SendPacket(new net.minecraft.src.Packet1Login(string.Empty, entityplayermp
					.entityId, worldserver.GetRandomSeed(), unchecked((byte)worldserver.worldProvider
					.worldType)));
				netserverhandler.SendPacket(new net.minecraft.src.Packet6SpawnPosition(chunkcoordinates
					.posX, chunkcoordinates.posY, chunkcoordinates.posZ));
				...
				mcServer.configManager.SendPacketToAllPlayers(new net.minecraft.src.Packet3Chat((
					new java.lang.StringBuilder()).Append("\xf7e").Append(entityplayermp.username).Append
					(" joined the game.").ToString()));
				mcServer.configManager.PlayerLoggedIn(entityplayermp);
				netserverhandler.TeleportTo(entityplayermp.posX, entityplayermp.posY, entityplayermp
					.posZ, entityplayermp.rotationYaw, entityplayermp.rotationPitch);
		...
		public virtual string GetUserAndIPString()
		{
			if (username != null)
			{
				return (new java.lang.StringBuilder()).Append(username).Append(" [").Append(netManager
					.GetRemoteAddress().ToString()).Append("]").ToString();
			}
			else
			{
