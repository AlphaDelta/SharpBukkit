...
{
	public class MapGenCaves : net.minecraft.src.MapGenBase
	{
		public MapGenCaves()
		{
		}
		...
		//            World
		protected internal virtual void Func_669_a(int i, int j, byte[] abyte0, double d, 
			double d1, double d2)
		{
			ReleaseEntitySkin(i, j, abyte0, d, d1, d2, 1.0F + rand.NextFloat() * 6F, 0.0F, 0.0F
				, -1, -1, 0.5D);
		}

		protected internal virtual void ReleaseEntitySkin(int i, int j, byte[] abyte0, double
			 d, double d1, double d2, float f, float f1, float f2, int k, int l, double d3)
		{
			double d4 = i * 16 + 8;
			double d5 = j * 16 + 8;
			float f3 = 0.0F;
			...
			for (; k < l; k++)
			{
				...
				if (!flag && k == j1 && f > 1.0F)
				{
					ReleaseEntitySkin(i, j, abyte0, d, d1, d2, ((float)random.NextDouble()) * 0.5F + 0.5F, f1 -
						 1.570796F, f2 / 3F, k, l, 1.0D);
					ReleaseEntitySkin(i, j, abyte0, d, d1, d2, ((float)random.NextDouble()) * 0.5F + 0.5F, f1 +
						 1.570796F, f2 / 3F, k, l, 1.0D);
					return;
				}
				...
				for (int j2 = d8; !flag2 && j2 < k1; j2++)
				{
					for (int l2 = d10; !flag2 && l2 < i2; l2++)
					{
						for (int i3 = l1 + 1; !flag2 && i3 >= d9 - 1; i3--)
						{
							...
							if (abyte0[j3] == net.minecraft.src.Block.WATER.ID || abyte0[j3] == net.minecraft.src.Block
								.STATIONARY_WATER.ID)
							{
								flag2 = true;
							}
					...
					for (int k3 = d10; k3 < i2; k3++)
					{
						...
						do
						{
							...
							if (d14 > -0.69999999999999996D && d12 * d12 + d14 * d14 + d13 * d13 < 1.0D)
							{
								...
								if (byte0 == net.minecraft.src.Block.GRASS.ID)
								{
									flag3 = true;
								}
								if (byte0 == net.minecraft.src.Block.STONE.ID || byte0 == net.minecraft.src.Block
									.DIRT.ID || byte0 == net.minecraft.src.Block.GRASS.ID)
								{
									if (i4 < 10)
									{
										abyte0[l3] = unchecked((byte)net.minecraft.src.Block.LAVA.ID);
									}
									else
									{
										abyte0[l3] = 0;
										if (flag3 && abyte0[l3 - 1] == net.minecraft.src.Block.DIRT.ID)
										{
											abyte0[l3 - 1] = unchecked((byte)net.minecraft.src.Block.GRASS.ID);
										}
									}
								}
							}
...
label0_break: ;
		...
		{
			int i1 = rand.Next(rand.Next(rand.Next(40) + 1) + 1);
			if (rand.Next(15) != 0)
			{
				i1 = 0;
			}
