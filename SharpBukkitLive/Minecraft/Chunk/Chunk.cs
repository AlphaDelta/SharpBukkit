...
namespace net.minecraft.src
{
    public class Chunk
    {
        ...
        public virtual void OnChunkUnload()
        {
            ...
            for (System.Collections.IEnumerator iterator = chunkTileEntityMap.Values.GetEnumerator(); iterator.MoveNext(); tileentity.Invalidate()) //TODO: Craftbukkit fix???
            {
            ...
            for (int i = 0; i < entities.Length; i++)
            {
                ...
                entities[i] = entities[i].Where(p => !(p is EntityPlayerMP && (((int)Math.Floor(p.posX)) >> 4 != this.xPosition || ((int)Math.Floor(p.posZ)) >> 4 != this.zPosition))).ToList();
                // CRAFTBUKKIT end
