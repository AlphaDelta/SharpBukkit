...
namespace net.minecraft.src
{
	public class WorldGenLiquids : net.minecraft.src.WorldGenerator
	{
		...
		public override bool Generate(net.minecraft.src.World world, SharpBukkitLive.SharpBukkit.SharpRandom random
			, int i, int j, int k)
		{
			if (world.GetBlockId(i, j + 1, k) != net.minecraft.src.Block.stone.blockID)
			{
				return false;
			}
			if (world.GetBlockId(i, j - 1, k) != net.minecraft.src.Block.stone.blockID)
			...
			int l = 0;
			if (world.GetBlockId(i - 1, j, k) == net.minecraft.src.Block.stone.blockID)
			{
				l++;
			}
			if (world.GetBlockId(i + 1, j, k) == net.minecraft.src.Block.stone.blockID)
			...
			if (world.GetBlockId(i, j, k - 1) == net.minecraft.src.Block.stone.blockID)
			{
				l++;
			}
			if (world.GetBlockId(i, j, k + 1) == net.minecraft.src.Block.stone.blockID)
