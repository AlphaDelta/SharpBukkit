...
{
	public class NoiseGeneratorPerlin : net.minecraft.src.NoiseGenerator
	{
		public NoiseGeneratorPerlin()
			: this(new SharpBukkitLive.SharpBukkit.SharpRandom())
		{
		}

		public NoiseGeneratorPerlin(SharpBukkitLive.SharpBukkit.SharpRandom random)
		{
		...
		public virtual double GenerateNoise(double d, double d1, double d2)
		{
			double d3 = d + xCoord;
			double d4 = d1 + yCoord;
			double d5 = d2 + zCoord;
			...
			return Lerp(d8, Lerp(d7, Lerp(d6, Grad(permutations[l1], d3, d4, d5), Grad(permutations
				[k2], d3 - 1.0D, d4, d5)), Lerp(d6, Grad(permutations[i2], d3, d4 - 1.0D, d5), Grad
				(permutations[l2], d3 - 1.0D, d4 - 1.0D, d5))), Lerp(d7, Lerp(d6, Grad(permutations
				[l1 + 1], d3, d4, d5 - 1.0D), Grad(permutations[k2 + 1], d3 - 1.0D, d4, d5 - 1.0D
				)), Lerp(d6, Grad(permutations[i2 + 1], d3, d4 - 1.0D, d5 - 1.0D), Grad(permutations
		...
		public virtual double Func_642_a(double d, double d1)
		{
			return GenerateNoise(d, d1, 0.0D);
		}

		public virtual void Func_646_a(double[] ad, double d, double d1, double d2, int i
			, int j, int k, double d3, double d4, double d5, double d6)
		{
			if (j == 1)
			{
				bool flag = false;
				...
				for (int i4 = 0; i4 < i; i4++)
				{
					...
					for (int l4 = 0; l4 < k; l4++)
					{
						...
						double d9 = Lerp(d17, Func_4102_a(permutations[j1], d14, d19), Grad(permutations[
							l1], d14 - 1.0D, 0.0D, d19));
						double d11 = Lerp(d17, Grad(permutations[j1 + 1], d14, 0.0D, d19 - 1.0D), Grad(permutations
							[l1 + 1], d14 - 1.0D, 0.0D, d19 - 1.0D));
				...
				for (int j6 = 0; j6 < k; j6++)
				{
					...
					for (int i7 = 0; i7 < j; i7++)
					{
						...
						if (i7 == 0 || k7 != i2)
						{
							...
							d13 = Lerp(d22, Grad(permutations[k2], d20, d26, d24), Grad(permutations[k3], d20
								 - 1.0D, d26, d24));
							d15 = Lerp(d22, Grad(permutations[l2], d20, d26 - 1.0D, d24), Grad(permutations[l3
								], d20 - 1.0D, d26 - 1.0D, d24));
							d16 = Lerp(d22, Grad(permutations[k2 + 1], d20, d26, d24 - 1.0D), Grad(permutations
								[k3 + 1], d20 - 1.0D, d26, d24 - 1.0D));
							d18 = Lerp(d22, Grad(permutations[l2 + 1], d20, d26 - 1.0D, d24 - 1.0D), Grad(permutations
								[l3 + 1], d20 - 1.0D, d26 - 1.0D, d24 - 1.0D));
						}
